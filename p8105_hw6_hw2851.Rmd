...
The dataset has `r nrow(birth_weight_df)` rows. We notice that after removing rows with NA values the row count is still `r nrow(na.omit(birth_weight_df))`, meaning that the dataset doesn't contain NA values.
We also notice variables "pnumsga" and "pnumlbw" are all 0 across the dataset by using the `unique` function, so we don't use these variables as predictors.

Low birth weight is a major public health problem. There is a large number of research literature studying the underlying factors that may affect birth weight, so we decide to choose the predictors based on existing research.
In particular, we referred to this article: Metgud, Chandra S., Vijaya A. Naik, and Maheshwar D. Mallapur. "Factors affecting birth weight of a newborn–a community based study in rural Karnataka, India." PloS one 7.7 (2012): e40040.
...
```
Then generate 5,000 bootstrap samples, fit a linear regression model tmax ~ tmin for each:
```{r}
bootstrap_results =
  weather_df %>%
...
Here we identify the 2.5% and 97.5% quantiles to provide a 95% confidence interval for $\hat{r}^{2}$:
```{r}
r_squared %>%
  summarize(
...
Then we plot the distribution of $\hat{r}^{2}$:
```{r}
r_squared %>%
  ggplot(aes(x = r.squared)) + geom_density()
```
It roughly follows normal distribution with mean around 0.913.
...
$\log(\hat{\beta_0} * \hat{\beta_1})$ also roughly follows normal distribution, with mean around 2.01.